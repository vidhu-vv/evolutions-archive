<script>
	...
	import { loadArchiveFromManifest } from '$lib/archive';

	/** @type {import('$lib/archive').Episode[]} */
	let episodes = [];
	/** @type {import('$lib/archive').Episode | null} */
	...
	$: {
		/** @type {Map<string, import('$lib/archive').Episode[]>} */
		const groupedEpisodes = new Map();

		for (const episode of episodes) {
			const label =
				episode.seasonNumber !== null ? `Season ${episode.seasonNumber}` : 'Other Recordings';
		...
		seasonGroups = Array.from(groupedEpisodes, ([label, grouped]) => ({
			label,
			episodes: grouped
		}));
	}
...
<div class="page-shell">
	<header class="topbar">
		...
		<div class="topbar-meta">
			<span>{source}</span>
			<span>{stats.totalEpisodes} recordings</span>
		</div>
	</header>
	...
	<section class="player-panel">
		...
		{#if activeEpisode}
			...
			<div class="feature-meta">
				...
				<span>{activeEpisode.source === 'cloudflare' ? 'R2 stream' : 'Local file'}</span>
			</div>
	...
	<section class="archive-panel">
		<div class="panel-heading">
			...
			<p class="panel-note">{stats.folders} collections</p>
		</div>
...
<style>
	:global(body) {
		...
		background:
			radial-gradient(circle at top, rgba(8, 126, 139, 0.08), transparent 24%),
			radial-gradient(circle at 80% 20%, rgba(255, 90, 95, 0.08), transparent 24%),
